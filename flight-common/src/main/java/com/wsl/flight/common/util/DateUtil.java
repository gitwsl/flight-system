...
public class DateUtil {
	 ...
	 * @return
	 */
	public static String getDateStr(long milSec) {
	 ...
	 * @return 周几
	 */
	public static String getWeekDayCN(int intDay) {
		String week = "未知";
		switch (intDay) {
			case 1:
				week = "星期日";
	...
	public static int getWeekDay(String strDay) {
		...
		strDate = String.valueOf(strDay).trim();
	 ...
	 * @return 排序结果
	 */
	public static List<? extends Date> orderDate(
			List<? extends Date> dates, int orderType) {
	...
	public static List<String> getQuarYear_YearMonth(Date dateObj, int count) {
		...
		for (int i = 0; i < count; i++) {
			CtolMonDates
					.add(ca.get(Calendar.YEAR)
							+ ""
							+ (ca.get(Calendar.MONTH) + 1 > 10 ? ca
	 ...
	 * @return
	 */
	public static String getNowdate(String type) throws Exception {
		Date dateValue = Calendar.getInstance().getTime();
		SimpleDateFormat sftime = new SimpleDateFormat(
				type);
		return sftime.format(dateValue);
...
//		System.out.println(getWeekDayCN(5));
	 ...
	 * @return 日期
	 */
	public static String addDate(int to) {
		try {
	 ...
	 * @return 日期
	 */
	public static String addMonth(int to) {
		try {
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			Calendar c = Calendar.getInstance();
